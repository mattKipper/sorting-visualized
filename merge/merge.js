...
var ColorSpectrum = (function () {
    function ColorSpectrum(hue, saturation_percent) {
        this.hue = hue;
        this.saturation_percent = saturation_percent;
    }
    ColorSpectrum.prototype.to_css = function (lightness_percent) {
...
// Define a spectrum for each merge type
var merge_spectrum = new ColorSpectrum(300, 75);
var merge_item_count = 128;
...
function merge_sort(arr, row) {
    ...
    var a = merge_sort(arr.slice(0, mid), row + 1);
    var b = merge_sort(arr.slice(mid, arr.length), row + 1);
...
function append_result(arr, row_num, sort_name, color) {
    // Check if a row exists at the given row. If not, create one
    var row_id = sort_name + "_row_" + String(row_num);
    ...
    for (var i = 0; i < arr.length; ++i) {
        ...
        var max_light = 90, min_light = 30;
        var lightness = (item / merge_item_count) * (max_light - min_light) + min_light;
        var color_css = color.to_css(lightness);
        item_node.style.backgroundColor = color_css;
        row.appendChild(item_node);
    }
}
window.onload = function () {
    var items = shuffled_range(merge_item_count);
    merge_sort(items);
};
